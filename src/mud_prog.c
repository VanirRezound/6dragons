...
int                     mprog_do_command( char *cmnd, CHAR_DATA *mob, CHAR_DATA *actor,
                                          OBJ_DATA *obj, void *vo, CHAR_DATA *rndm, bool ignore,
                                          bool ignore_ors );
...
char                   *strip_tilde( const char *str );
...
char                   *mprog_one_command( char *argument, char *arg_first )
{
    ...
    while ( *argument != '\0' || ++count >= 255 ) {
        if ( *argument == '\n' || *argument == '\r' ) {
            ...
            if ( lch == '+' ) {                        /* If the last char was a + we
                                                        * want to continue */
                if ( newappend ) {
                    arg_first--;                       /* Need to replace the + with a
                                                        * space */
                    *arg_first = ' ';
                    arg_first++;
                    newappend = FALSE;
...
char                   *parse_var( const char *cmnd, char_data * mob, char_data * actor,
 ...
 */
int mprog_do_ifcheck( char *ifcheck, CHAR_DATA *mob, CHAR_DATA *actor, OBJ_DATA *obj, void *vo,
                      CHAR_DATA *rndm )
{
    char                    buf[MSL],
                            opr[MIL];
  ...
  doneargs:
    ...
    *p = '\0';
    ...
    if ( !str_cmp( chck, "economy" ) ) {
        ...
        if ( !idx ) {
            if ( !mob->in_room ) {
                progbug( mob, "'economy' ifcheck: mob in NULL room with no room vnum argument [%s]",
                         mob->name );
                return BERR;
            }
            room = mob->in_room;
    ...
    if ( chkchar ) {
        ...
        if ( !str_cmp( chck, "mobinvislevel" ) ) {
            return ( IS_NPC( chkchar ) ? mprog_veval( chkchar->mobinvis, opr, atoi( rval ), mob ) :
                     FALSE );
        }
        ...
        if ( !str_cmp( chck, "faith" ) )               // faith support to progs. -Taon
        {
            if ( IS_NPC( chkchar ) )
                return FALSE;

            return mprog_veval( chkchar->faith, opr, atoi( rval ), mob );
        }
        if ( !str_cmp( chck, "focus" ) )               // focus support to progs. -Taon
        {
            if ( IS_NPC( chkchar ) )
                return FALSE;

            return mprog_veval( chkchar->focus_level, opr, atoi( rval ), mob );
        }
        if ( !str_cmp( chck, "position" ) ) {
            return mprog_veval( chkchar->position, opr, atoi( rval ), mob );
        }
        if ( !str_cmp( chck, "ishelled" ) ) {
            return IS_NPC( chkchar ) ? FALSE : mprog_veval( chkchar->pcdata->release_date, opr,
                                                            atoi( rval ), mob );
        }
        ...
        if ( !str_cmp( chck, "multi" ) ) {
            ...
            for ( ch = first_char; ch; ch = ch->next )
                if ( !IS_NPC( chkchar ) && !IS_NPC( ch ) && ch->desc && chkchar->desc
                     && VLD_STR( ch->desc->host ) && VLD_STR( chkchar->desc->host )
...
*/
    ...
    if ( !str_cmp( chck, "charcount" ) ) {             /* -- Gorog */
        ...
        for ( tch = room ? room->first_person : NULL; tch; tch = tch->next_in_room )
            ...
            if ( ( ( !IS_NPC( tch ) ) && get_trust( tch ) < LEVEL_IMMORTAL ) || IS_NPC( tch ) ) /* mortal
                                                                                                 * * or
                                                                                                 * mob */
                count++;
        return mprog_veval( count, opr, atoi( rval ), mob );
 ...
 */
void mprog_translate( char ch, char *t, CHAR_DATA *mob, CHAR_DATA *actor, OBJ_DATA *obj, void *vo,
                      CHAR_DATA *rndm )
{
    ...
    switch ( ch ) {
        ...
        case 'p':
            if ( v_obj && !obj_extracted( v_obj ) )
                can_see_obj( mob, v_obj ) ? one_argument( v_obj->name, t ) : strcpy( t,
                                                                                     "something" );
            else
                strcpy( t, "something" );
            break;
        ...
        case 'P':
            if ( v_obj && !obj_extracted( v_obj ) )
                can_see_obj( mob, v_obj ) ? strcpy( t, v_obj->short_descr ) : strcpy( t,
                                                                                      "something" );
            else
                strcpy( t, "something" );
            break;
                   ...
                   bool single_step )
{
    ...
    if ( ++prog_nest > MAX_PROG_NEST ) {
        progbug( mob, "max_prog_nest exceeded. [%s] [%d]: prog_nest: %d", mob->name,
                 mob->pIndexData->vnum, prog_nest );
        --prog_nest;
        return;
    }
    /*
...
//    command_list = mprog_next_command(command_list);
        ...
        if ( !str_prefix( "mppause", cmnd ) ) {
            ...
            if ( ( ifstate[iflevel][IN_IF] == TRUE && ifstate[iflevel][DO_IF] == FALSE )
                 || ( ifstate[iflevel][IN_ELSE] == TRUE && ifstate[iflevel][DO_ELSE] == FALSE ) ) {
                continue;
            }
            else {
...
//VOLK: Added support for faster/slower mppauses!
                ...
                if ( actor && actor->pcdata && actor->pcdata->textspeed ) {
...
// Now, we need a formula to make sure timers aren't TOO fast or TOO slow.
                 ...
                 */
                if ( mppause->mob && mppause->mob->pIndexData
                     && mppause->mob->pIndexData->vnum == 3 ) {
         ...
         */
        result =
            mprog_do_command( cmnd, mob, actor, obj, vo, rndm,
                              ( ifstate[iflevel][IN_IF] && !ifstate[iflevel][DO_IF] )
                              || ( ifstate[iflevel][IN_ELSE]
                                   && !ifstate[iflevel][DO_ELSE] ), ( ignorelevel > 0 ) );
        /*
        ...
        switch ( result ) {
            ...
            case IFTRUE:
                ...
                iflevel++;
                if ( iflevel == MAX_IFS ) {
                    progbug( mob, "Maximum nested ifs exceeded [name: %s] [vnum: %d]", mob->name,
                             mob->pIndexData->vnum );
                    --prog_nest;
                    return;
                }
            ...
            case ORTRUE:
                ...
                if ( !ifstate[iflevel][IN_IF] ) {
                    progbug( mob, "Unmatched or [name: %s] [vnum: %d]", mob->name,
                             mob->pIndexData->vnum );
                    --prog_nest;
                    return;
                }
            ...
            case FOUNDELSE:
                ...
                if ( ifstate[iflevel][IN_ELSE] ) {
                    progbug( mob, "Found else in an else section [name: %s] [vnum: %d]", mob->name,
                             mob->pIndexData->vnum );
                    --prog_nest;
                    return;
                }
                if ( !ifstate[iflevel][IN_IF] ) {
                    progbug( mob, "Unmatched else [name: %s] [vnum: %d]", mob->name,
                             mob->pIndexData->vnum );
                    --prog_nest;
                    return;
                }
            ...
            case FOUNDENDIF:
                 ...
                 */
                if ( !( ifstate[iflevel][IN_IF] || ifstate[iflevel][IN_ELSE] ) ) {
                    progbug( mob, "Unmatched endif [name:%s] [vnum: %d]", mob->name,
                             mob->pIndexData->vnum );
                    --prog_nest;
                    return;
                }
            ...
            case IFIGNORED:
                ...
                if ( !( ifstate[iflevel][IN_IF] || ifstate[iflevel][IN_ELSE] ) ) {
                    progbug( mob,
                             "Parse error, ignoring if while not in if or else [name: %s] [vnum: %d]",
            ...
            case ORIGNORED:
                ...
                if ( ignorelevel == 0 ) {
                    progbug( mob, "Parse error, mistakenly ignoring or [name: %s] [vnum: %d]",
                             mob->name, mob->pIndexData->vnum );
                    --prog_nest;
                    return;
                }
 ...
 */
int mprog_do_command( char *cmnd, CHAR_DATA *mob, CHAR_DATA *actor, OBJ_DATA *obj, void *vo,
...
void mppause_update(  )
{
    ...
    mppause = first_mppause;
    while ( mppause ) {                                /* Find progs to continue */
        if ( --mppause->timer <= 0 ) {
            ...
            mprog_driver( mppause->com_list, mppause->mob, mppause->actor, mppause->obj,
                          mppause->vo, mppause->single_step );
 ...
 */
void mprog_wordlist_check( char *arg, CHAR_DATA *mob, CHAR_DATA *actor, OBJ_DATA *obj, void *vo,
                           int type )
{
    ...
    if ( IS_NPC( mob ) && can_see( mob, ch ) && HAS_PROG( mob->pIndexData, GIVE_PROG ) ) {
        ...
        for ( mprg = mob->pIndexData->mudprogs; mprg; mprg = mprg->next ) {
            ...
            if ( mprg->type == GIVE_PROG
                 && ( !str_cmp( obj->name, mprg->arglist ) || !str_cmp( obj->name, buf )
                      || !str_cmp( "all", buf ) || ( vnum != -1
    ...
    while ( ( runner = room_act_list ) != NULL ) {
        ...
        while ( ( mpact = room->mpact ) != NULL ) {
            if ( mpact->ch->in_room == room )
                rprog_wordlist_check( mpact->buf, supermob, mpact->ch, mpact->obj, mpact->vo,
                                      ACT_PROG, room );
...
void obj_act_update( void )
{
    ...
    while ( ( runner = obj_act_list ) != NULL ) {
        ...
        while ( ( mpact = obj->mpact ) != NULL ) {
            oprog_wordlist_check( mpact->buf, supermob, mpact->ch, mpact->obj, mpact->vo, ACT_PROG,
                                  obj );
