...
int load_imm_host(  )
{
    ...
    if ( ( fp = FileOpen( IMM_HOST_FILE, "r" ) ) == NULL ) {
...
int fread_imm_host( FILE * fp, IMMORTAL_HOST * data )
{
    ...
    for ( ;; ) {
        ...
        switch ( UPPER( word[0] ) ) {
            case 'E':
                if ( !str_cmp( word, "End" ) )
                    return rNONE;
            case 'H':
                KEY( "Host", data->host, fread_string( fp ) );
                break;
            case 'N':
                KEY( "Name", data->name, fread_string( fp ) );
                break;
            case 'P':
        ...
        if ( !fMatch ) {
            bug( "Fread_immortal_host: no match: %s", word );
            fread_to_eol( fp );
        }
    }
    return rNONE;
...
void do_write_imm_host( void )
{
    ...
    if ( ( fp = FileOpen( IMM_HOST_FILE, "w" ) ) == NULL ) {
        bug( "%s", "Do_write_imm_host: FileOpen" );
