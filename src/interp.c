...
/* Volk - spice up socials by targetting objects */
        OBJ_DATA               *obj,
                               *obj_next;

        for ( ( obj = ch->first_carrying ); obj; obj = obj_next ) {
            obj_next = obj->next_content;
            if ( nifty_is_name( obj->name, arg ) || nifty_is_name_prefix( arg, obj->name ) ) {
                invobj = TRUE;
                break;
            }
        }
        if ( !invobj ) {                               /* Let's check objects in room now 
                                                        */
        ...
        /*
         * Volk - need to make new function :( 
         */
        if ( invobj || roomobj ) {
            act( AT_SOCIAL, social->char_found, ch, obj, NULL, TO_CHAR );
            act( AT_SOCIAL, social->others_found, ch, obj, NULL, TO_NOTVICT );
            return TRUE;
        }

        if ( !victim && !invobj && !roomobj )
            send_to_char( "They aren't here.\r\n", ch );
    }
/*
  else if((victim = get_char_room(ch, arg)) != NULL && !IS_NPC(victim) && (victim->in_room->vnum != ch->in_room->vnum))
  {
    if(!social->vict_found || !social->char_found)
    {
      send_to_char("Incomplete social, try again soon.\r\n", ch);
      bug("%s: Incomplete Social %s, has NULL vict_found or NULL char_found", __FUNCTION__, social->name);
      return TRUE;
    }
    strcpy(buf, "From afar...");
    mudstrlcat(buf, social->vict_found, MSL);
    act(AT_SOCIAL, buf, ch, NULL, victim, TO_VICT);
    strcpy(buf, "From afar...");
    mudstrlcat(buf, social->char_found, MSL);
