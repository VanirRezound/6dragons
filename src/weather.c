...
void InitializeWeatherMap( void )
{
    ...
    for ( y = 0; y < WEATHER_SIZE_Y; y++ ) {
        for ( x = 0; x < WEATHER_SIZE_X; x++ ) {
            ...
            cell->climate = number_chance( 0, 10 );
            cell->hemisphere = number_chance( 0, 1 );

            cell->temperature = number_range( -30, 100 );
            cell->pressure = number_chance( 0, 100 );
            cell->cloudcover = number_chance( 0, 100 );
            cell->humidity = number_chance( 0, 100 );
            cell->precipitation = number_chance( 0, 100 );
            cell->windSpeedX = number_range( -100, 100 );
            cell->windSpeedY = number_range( -100, 100 );
            cell->energy = number_chance( 0, 100 );
        }
    }
}
            ...
            for ( d = first_descriptor; d; d = d->next ) {
                if ( d->connected == CON_PLAYING ) {
                    if ( d->character && ( d->character->in_room->area == pArea )
                        ...
                        if ( d->character->in_room->sector_type == SECT_DESERT ) {
                            short                   chance;

                            chance = number_range( 1, 6 );
                            if ( chance == 1 )
                                send_to_char( "&OThe heat is almost unbearable.&D\r\n",
                                              d->character );
                            else if ( chance == 2 )
                                send_to_char( "&OIt's very hot...&D\r\n", d->character );
                            else if ( chance == 3 )
                                send_to_char( "&OIt's hot...&D\r\n", d->character );
                            else if ( chance == 4 )
                                send_to_char( "&OThe dry heat from the sun cooks you!&D\r\n",
                                              d->character );
                        }
                        if ( d->character->in_room->sector_type != SECT_DESERT ) {
        ...
        for ( x = 0; x < WEATHER_SIZE_X; x++ ) {
            ...
            if ( isTorrentialDownpour( getPrecip( cell ) ) ) {
                ...
                if ( DropsBelowThreshold( cell->temperature, delta->temperature, 32 ) )
                    ...
                    WeatherMessage
                        ( "&WLarge snow flakes continue to fall obscuring visibility.&D\r\n", x,
                          y );
                else if ( isStormy( getEnergy( cell ) ) )
                    WeatherMessage
                ...
                if ( DropsBelowThreshold( cell->temperature, delta->temperature, 32 ) )
                    ...
                    if ( isStormy( getEnergy( cell ) ) )
                        WeatherMessage
                            ( "&BThe steady rain eases up a bit.&D\r\n&YLightning and thunder illuminate the sky and shake the ground.&D\r\n",
                              x, y );
                    else
            ...
            if ( cell->climate == CLIMATE_DESERT ) {
                cell->precipitation = number_chance( 0, 1 );
                cell->temperature = number_chance( 90, 120 );
                cell->cloudcover = number_chance( 0, 1 );
            }
    ...
    for ( y = 0; y < WEATHER_SIZE_Y; y++ ) {
        for ( x = 0; x < WEATHER_SIZE_X; x++ ) {
            ...
            if ( cell->precipitation > 40 )
                ...
                delta->humidity += number_chance( 0, 3 );
            ...
            for ( dy = -1; dy <= 1; ++dy ) {
                for ( dx = -1; dx <= 1; ++dx ) {
                    ...
                    if ( ( cell->windSpeedX < 0 && dx < 0 ) || ( cell->windSpeedX > 0 && dx > 0 )
                         || ( cell->windSpeedY < 0 && dy < 0 ) || ( cell->windSpeedY > 0
                                                                    && dy > 0 ) ) {
...
void EnforceClimateConditions( void )
{
    ...
    for ( y = 0; y < WEATHER_SIZE_Y; y++ ) {
        for ( x = 0; x < WEATHER_SIZE_X; x++ ) {
            ...
            if ( cell->climate == CLIMATE_DESERT ) {
                cell->temperature = number_range( 90, 140 );
                cell->pressure = number_range( 80, 90 );
                cell->cloudcover = number_range( 1, 5 );
                cell->humidity = number_range( 10, 30 );
                cell->precipitation = number_range( 1, 3 );
        ...
        for ( x = 0; x < WEATHER_SIZE_X; x++ ) {
            ...
            if ( cell->hemisphere == HEMISPHERE_NORTH ) {
                if ( time_info.season == SEASON_SPRING ) {
                    if ( cell->climate == CLIMATE_RAINFOREST ) {
                        cell->temperature = number_chance( 70, 90 );
                        cell->pressure = number_chance( 30, 60 );
                        cell->cloudcover = number_chance( 50, 70 );
                        cell->humidity = number_chance( 70, 100 );
                        cell->precipitation = number_chance( 70, 100 );
                        cell->windSpeedX = number_range( -20, 20 );
                        cell->windSpeedY = number_range( -20, 20 );
                        cell->energy = number_chance( 0, 100 );
                    }
                    else if ( cell->climate == CLIMATE_SAVANNA ) {
                        cell->temperature = number_chance( 60, 80 );
                        cell->pressure = number_chance( 30, 50 );
                        cell->cloudcover = number_chance( 20, 40 );
                        cell->humidity = number_chance( 60, 80 );
                        cell->precipitation = number_chance( 60, 80 );
                        cell->windSpeedX = number_range( -20, 20 );
                        cell->windSpeedY = number_range( -20, 20 );
                        cell->energy = number_chance( 0, 100 );
                    }
...
void do_setweather( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !str_cmp( arg3, "climate" ) ) {
        ...
        if ( !str_cmp( arg4, "none" ) ) {
            cell->climate = 0;
            send_to_char( "Cell Climate removed.\r\n", ch );
            return;
        }
    ...
    if ( !str_cmp( arg3, "temperature" ) ) {
        if ( !arg4 || arg4[0] == '\0' ) {
            send_to_char( "Usage: setweather <x> <y> temperature <degrees>\r\n", ch );
            return;
        }
