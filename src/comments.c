...
void do_comment( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !str_cmp( arg, "remove" ) ) {
        ...
        if ( ( get_trust( victim ) >= get_trust( ch ) ) || ( get_trust( ch ) < LEVEL_AJ_SGT ) ) {
            send_to_char( "You're not of the right caliber to do this...\r\n", ch );
            return;
        }
        ...
        for ( pnote = victim->comments; pnote; pnote = pnote->next ) {
            vnum++;
            if ( ( LEVEL_AJ_SGT <= get_trust( ch ) ) && ( vnum == anum ) ) {
    ...
    send_to_char( "&cSyntax: &Ccomment <option> <player>\r\n", ch );
    send_to_char( "&cOption being: &Cwrite, subject, post, list, read, remove\r\n", ch );
