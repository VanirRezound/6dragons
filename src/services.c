/****************************************************************************
 * Planes Pre-release (C) 2000-2006 by Telara/Sharmair  based on SMAUG 1.4a *
 * ------------------------------------------------------------------------ *
...
#include <windows.h>                                   /* for service and thread routines 
                                                        */
#pragma warning(disable: 4057 4505)
static char             rcsid[] = "$Id: services.c,v 1.5 1998/09/11 04:34:10 shaddai Exp $";    /* RCS 
                                                                                                 * revision 
                                                                                                 * id 
                                                                                                 */
...
static VOID service_main( DWORD dwArgc, LPTSTR * lpszArgv )
{
    ...
    threadHandle = ( HANDLE ) _beginthreadex( NULL,    // security attributes
                                              ...
                                              worker_thread, NULL,  // argument to
                                              // thread
                                              0,       // thread creation flags
                                              ( unsigned * ) &TID );    // pointer to
    // thread ID
...
//
static BOOL ReportStatusToSCMgr( DWORD dwCurrentState, DWORD dwWin32ExitCode, DWORD dwCheckPoint,
                                 DWORD dwWaitHint )
{
    BOOL                    fResult;
    ...
    // 
    if ( !( fResult = SetServiceStatus( sshStatusHandle,    // service reference
                                        // handle
...
*/
    ...
    service = CreateService( SCmanager, THIS_SERVICE, THIS_SERVICE_DISPLAY, SERVICE_ALL_ACCESS, SERVICE_WIN32_OWN_PROCESS, SERVICE_AUTO_START, SERVICE_ERROR_NORMAL, fullfilename, NULL,    // no 
                                                                                                                                                                                            // 
                             // 
                             // 
                             // 
...
*/
    ...
    if ( ( status = get_service_status( &svcstatus, TRUE ) ) != 0 )
...
*/
    ...
    if ( ( status = get_service_status( &svcstatus, FALSE ) ) != 0 )
        return service_error( status, "Unable to access service details" );
...
static char            *convert_error( DWORD error )
{
    ...
    if ( !FormatMessage
         ( FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_ALLOCATE_BUFFER |
           FORMAT_MESSAGE_IGNORE_INSERTS, NULL, error, LANG_NEUTRAL, ( LPTSTR ) & formattedmsg, 0,
           NULL ) ) {
