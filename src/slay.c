...
/** Function: do_slay
  ...
  * Ported to Smaug 1.02a by: Samson <snippets@alsherok.net>
...
void do_slay( CHAR_DATA *ch, char *argument )
{
    ...
    if ( get_trust( victim ) >= get_trust( ch ) ) {
        ...
        act( AT_RED, "$n tried to slay you against your wishes.", ch, NULL, victim, TO_VICT );
        return;
    }

    if ( type[0] == '\0' ) {
        act( AT_IMMORT, "You brutally slayed $N!", ch, NULL, victim, TO_CHAR );
...
void do_destroyslay( CHAR_DATA *ch, char *argument )
{
    ...
    for ( pslay = first_slay; pslay; pslay = pslay->next ) {
        if ( !str_cmp( arg, pslay->type ) ) {
            ...
            if ( VLD_STR( pslay->owner ) )
                STRFREE( pslay->owner );
            if ( VLD_STR( pslay->type ) )
                STRFREE( pslay->type );
            if ( VLD_STR( pslay->cmsg ) )
                STRFREE( pslay->cmsg );
            if ( VLD_STR( pslay->vmsg ) )
                STRFREE( pslay->vmsg );
            if ( VLD_STR( pslay->rmsg ) )
                STRFREE( pslay->rmsg );
