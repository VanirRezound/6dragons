...
void save_deity( DEITY_DATA * deity )
{
    ...
    if ( !( fp = FileOpen( filename, "w" ) ) ) {
        bug( "%s: failed to open %s for writing.", __FUNCTION__, filename );
        perror( filename );
        return;
    }
...
void load_deity( void )
{
    ...
    if ( !( fpList = FileOpen( deitylist, "r" ) ) ) {
        perror( deitylist );
        exit( 1 );
    }
    ...
    for ( ;; ) {
        ...
        if ( filename[0] == '\0' ) {
            bug( "load_deity: EOF encountered reading file!" );
            break;
        }
        ...
        if ( !load_deity_file( filename ) )
            bug( "load_deity: Cannot load deity file: %s", filename );
    }
    FileClose( fpList );
...
void do_setdeity( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' ) {
        ...
        send_to_char( "scorpse savatar sdeityobj srecall\r\r\n\n", ch );
    ...
    if ( !str_cmp( arg2, "filename" ) ) {
        ...
        STRFREE( deity->filename );
        deity->filename = STRALLOC( argument );
        send_to_char( "Done.\r\n", ch );
        save_deity( deity );
        write_deity_list(  );
        return;
    ...
    if ( !str_cmp( arg2, "class" ) ) {
        ...
        if ( ( deity->Class < 0 ) || ( deity->Class >= MAX_PC_CLASS ) )
            deity->Class = -1;
        send_to_char( "Deity class now set.\r\n", ch );
        save_deity( deity );
        return;
    }
...
void do_showdeity( CHAR_DATA *ch, char *argument )
{
    ...
    ch_printf( ch, "Deity: %s\r\nFilename: %s\r\nDescription:\r\n%s\r\n", deity->name,
               deity->filename, deity->description );
    ch_printf( ch, "Alignment: %-7dNpcrace: %-9sNpcfoe: %s\r\n", deity->alignment,
               ( deity->npcrace < 0
                 || deity->npcrace > MAX_NPC_RACE ) ? "none" : npc_race[deity->npcrace],
               ( deity->npcfoe < 0
                 || deity->npcfoe > MAX_NPC_RACE ) ? "none" : npc_race[deity->npcfoe] );
    ch_printf( ch, "Race: %-12sClass: %-11sSex: %-13sRace2: %s\r\n",
               ( deity->race < 0
                 || deity->race > MAX_PC_RACE ) ? "none" : race_table[deity->race]->race_name,
               ( deity->Class < 0
                 || deity->Class > MAX_PC_CLASS ) ? "none" : class_table[deity->Class]->who_name,
               deity->sex == -1 ? "none" : deity->sex == SEX_MALE ? "male" : deity->sex ==
               SEX_FEMALE ? "female" : "neutral", ( deity->race2 < 0
                                                    || deity->race2 >
                                                    MAX_PC_RACE ) ? "none" : npc_race[deity->
                                                                                      race2] );
    ch_printf( ch, "Objstat: %-9dWorshippers: %d\r\n", deity->objstat, deity->worshippers );
    ch_printf( ch, "Affectednum: %-5dElementnum: %-6dSusceptnum: %d\r\n", deity->affectednum,
               deity->elementnum, deity->susceptnum );
    ...
    ch_printf( ch, "Aid: %-13dSteal: %-11dBackstab: %-8dDie: %d\r\n", deity->aid, deity->steal,
               deity->backstab, deity->die );
...
void do_makedeity( CHAR_DATA *ch, char *argument )
{
    ...
    LINK( deity, first_deity, last_deity, next, prev );
    deity->name = STRALLOC( argument );
    deity->filename = STRALLOC( strlower( argument ) );
    write_deity_list(  );
    save_deity( deity );
...
void do_devote( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' ) {
        send_to_char( "Syntax: Devote < deity >\r\n", ch );
    ...
    if ( !str_cmp( arg, "none" ) ) {
        ...
        af.modifier = -4;
        af.duration = number_range( 25, 50 );
...
*/
    ...
    act( AT_MAGIC, "Body and soul, you devote yourself to $t!", ch, ch->pcdata->deity_name, NULL,
         TO_CHAR );
    ++ch->pcdata->deity->worshippers;
    save_deity( ch->pcdata->deity );
    save_char_obj( ch );
...
void do_supplicate( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !str_cmp( arg, "corpse" ) && !IS_PKILL( ch ) ) {
        ...
        for ( obj = first_object; obj; obj = obj->next ) {
            if ( obj->in_room && !str_cmp( buf2, obj->short_descr )
                ...
                if ( IS_SET( obj->in_room->room_flags, ROOM_NOSUPPLICATE ) ) {
                    act( AT_MAGIC, "The image of your corpse appears, but suddenly wavers away.",
                         ch, NULL, NULL, TO_CHAR );
                    return;
                }
                act( AT_MAGIC, "Your corpse appears suddenly, surrounded by a divine presence...",
                     ch, NULL, NULL, TO_CHAR );
                act( AT_MAGIC, "$n's corpse appears suddenly, surrounded by a divine force...", ch,
                     NULL, NULL, TO_ROOM );
                obj_from_room( obj );
        ...
        ch->pcdata->favor += ch->pcdata->deity->scorpse;    /* Changed to + since they
                                                             * just set them to - on the 
                                                             * mud */
    ...
    if ( !str_cmp( arg, "avatar" ) ) {
        ...
        ch->pcdata->favor += ch->pcdata->deity->savatar;    /* Changed to += since -
                                                             * online */
    ...
    if ( !str_cmp( arg, "object" ) ) {
        ...
        ch->pcdata->favor += ch->pcdata->deity->sdeityobj;  /* Changed to += */
    ...
    if ( !str_cmp( arg, "recall" ) ) {
        ...
        act( AT_MAGIC, "$n appears in the room from a column of divine mist.", ch, NULL, NULL,
             TO_ROOM );
        do_look( ch, ( char * ) "auto" );
        ch->pcdata->favor += ch->pcdata->deity->srecall;    /* Changed to += */
...
void adjust_favor( CHAR_DATA *ch, int field, int mod )
{
    ...
    // Removed mod divider at the request of Greetmir. Now works to his
    // standards. -Taon
    ...
    switch ( field ) {
        case 0:
            ch->pcdata->favor += number_fuzzy( ch->pcdata->deity->flee );
            break;
        case 1:
            ch->pcdata->favor += number_fuzzy( ch->pcdata->deity->flee_npcrace );
            break;
        case 2:
            ch->pcdata->favor += number_fuzzy( ch->pcdata->deity->kill );
            break;
        case 3:
            ch->pcdata->favor += number_fuzzy( ch->pcdata->deity->kill_npcrace );
            break;
        case 4:
            ch->pcdata->favor += number_fuzzy( ch->pcdata->deity->kill_magic );
            break;
        case 5:
            ch->pcdata->favor += number_fuzzy( ch->pcdata->deity->sac );
            break;
        case 6:
            ch->pcdata->favor += number_fuzzy( ch->pcdata->deity->bury_corpse );
            break;
        case 7:
            ch->pcdata->favor += number_fuzzy( ch->pcdata->deity->aid_spell );
            break;
        case 8:
            ch->pcdata->favor += number_fuzzy( ch->pcdata->deity->aid );
            break;
        case 9:
            ch->pcdata->favor += number_fuzzy( ch->pcdata->deity->steal );
            break;
        case 10:
            ch->pcdata->favor += number_fuzzy( ch->pcdata->deity->backstab );
            break;
        case 11:
            ch->pcdata->favor += number_fuzzy( ch->pcdata->deity->die );
            break;
        case 12:
            ch->pcdata->favor += number_fuzzy( ch->pcdata->deity->die_npcrace );
            break;
        case 13:
            ch->pcdata->favor += number_fuzzy( ch->pcdata->deity->spell_aid );
            break;
        case 14:
            ch->pcdata->favor += number_fuzzy( ch->pcdata->deity->dig_corpse );
            break;
        case 15:
            ch->pcdata->favor += number_fuzzy( ch->pcdata->deity->die_npcfoe );
            break;
        case 16:
            ch->pcdata->favor += number_fuzzy( ch->pcdata->deity->flee_npcfoe );
            break;
        case 17:
            ch->pcdata->favor += number_fuzzy( ch->pcdata->deity->kill_npcfoe );
            break;
        default:
            log_string( "Error: default switch reached in adjust_favor." );
            break;
    }
    ch->pcdata->favor = URANGE( -2500, ch->pcdata->favor, 2500 );
...
void do_remains( CHAR_DATA *ch, char *argument )
{
    ...
    for ( obj = first_object; obj; obj = obj->next ) {
        if ( obj->in_room && !str_cmp( buf, obj->short_descr )
             && ( obj->pIndexData->vnum == OBJ_VNUM_CORPSE_PC ) ) {
            found = TRUE;
            pager_printf( ch, "\r\n  - at %s will endure for %d ticks", obj->in_room->name,
                          obj->timer );
        }
    }
    if ( !found )
